...
class LinearRegressionLitModule(LightningModule):
    ...
    def on_validation_epoch_end(self) -> None:
        ...
        acc = self.val_r2.compute()  # get current val acc
        self.val_acc_best(acc)  # update best so far val acc
        # log `val_acc_best` as a value through `.compute()` method, instead of as a metric object
        # otherwise metric would be reset by lightning after each epoch
        self.log("val/r2_best", self.val_acc_best.compute(), sync_dist=True, prog_bar=True)
...
if __name__ == "__main__":
    _ = LinearRegressionLitModule(None, None, None, None)
